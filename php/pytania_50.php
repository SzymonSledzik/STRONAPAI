	...
	<form method="POST" value="POST">
		...
		function print_data()
		{
			...
			for ($i = 1; $i <= 10; $i++) {
				for ($z = 0; $z < sizeof($char_array); $z++) {
					...
					if (isset($_POST["checkbox" . $i . $char_array[$z]])) {
						if ($_POST["checkbox" . $i . $char_array[$z]] == "True") $checkbox_print .= "checked";
					}

					$checkbox_print .= '><label for="nie' . $i . $char_array[$z] . '" class="label">Nie</label>
		...
		function submit_data()
		{
			...
			if ($error == 1) {
				...
				$maxVal = max($points);
				//search key fo the highest valueĹ›
				$maxKey = array_search($maxVal, $points);
				$results = "Najlepszy wynik uzyskaďż˝eďż˝ w $maxKey: $maxVal";
				if ($maxKey == "K") {
					echo "K";
				}
				if ($maxKey == "S") {
